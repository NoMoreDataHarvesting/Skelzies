...
enum ChalkRenderer {
    ...
    private static func drawAsphalt(_ ctx: CGContext, size: CGSize, rng: inout ChalkRNG) {
        ...
        if let grad = CGGradient(colorsSpace: CGColorSpaceCreateDeviceRGB(), colors: colors, locations: [0, 1]) {
            ...
            ctx.drawRadialGradient(grad, startCenter: center, startRadius: 120,
                                   endCenter: center, endRadius: 900,
    ...
    private static func drawBoard(_ ctx: CGContext, rng: inout ChalkRNG) {
        // Outer chalk square
        chalkRect(ctx, Layout.boardRect, width: 4.6, alpha: 0.9, rng: &rng)

        // Numbered boxes with outward-facing digits
        for box in Layout.boxes {
            chalkRect(ctx, box.rect, width: 4.2, alpha: 0.88, rng: &rng)
            let baseSize: CGFloat = box.n == 13 ? 38 : (box.rect.width > 85 * Layout.unit ? 46 : 34)
            let fontSize = baseSize * Layout.unit
        ...
        chalkRect(ctx, Layout.deadFrameRect, width: 3.4, alpha: 0.85, rng: &rng)
        let f = Layout.deadFrameRect, t = Layout.box13Rect
        chalkLine(ctx, from: CGPoint(x: f.minX, y: f.minY), to: CGPoint(x: t.minX, y: t.minY), width: 3.2, rng: &rng)
        chalkLine(ctx, from: CGPoint(x: f.maxX, y: f.minY), to: CGPoint(x: t.maxX, y: t.minY), width: 3.2, rng: &rng)
        chalkLine(ctx, from: CGPoint(x: f.maxX, y: f.maxY), to: CGPoint(x: t.maxX, y: t.maxY), width: 3.2, rng: &rng)
        chalkLine(ctx, from: CGPoint(x: f.minX, y: f.maxY), to: CGPoint(x: t.minX, y: t.maxY), width: 3.2, rng: &rng)
        ...
        // "Start" — chalked beside the cap cluster, reading bottom-to-top
        let sp = Layout.startImage
        drawChalkText(ctx, "Start", at: CGPoint(x: sp.x - 46, y: sp.y + 70), size: 28, rotation: -.pi / 2)
        chalkLine(ctx, from: CGPoint(x: sp.x - 28, y: sp.y + 24), to: CGPoint(x: sp.x - 28, y: sp.y + 118), width: 2.8, alpha: 0.8, rng: &rng)
    }
    ...
    static func capTexture(colorIndex i: Int, weight: CapWeight) -> SKTexture {
        let d: CGFloat = 44
        let format = UIGraphicsImageRendererFormat()
        format.scale = 2
        let image = UIGraphicsImageRenderer(size: CGSize(width: d, height: d), format: format).image { rc in
            ...
            // Center fill varies by weight class
            let inner = CGRect(x: c.x - R * 0.55, y: c.y - R * 0.55, width: R * 1.1, height: R * 1.1)
            switch weight {
            case .welterweight:
                // Classic wax fill
            ...
            case .lightweight:
                ...
                ctx.strokeEllipse(in: inner)
            case .heavyweight:
                // Quarter-loaded — silver coin with a ridged edge
                let coin = inner.insetBy(dx: -1, dy: -1)
                let coinColors = [UIColor(white: 0.92, alpha: 1).cgColor,
                                  UIColor(white: 0.72, alpha: 1).cgColor,
                                  UIColor(white: 0.50, alpha: 1).cgColor] as CFArray
                if let g = CGGradient(colorsSpace: CGColorSpaceCreateDeviceRGB(),
                                      colors: coinColors, locations: [0, 0.7, 1]) {
                    ctx.saveGState()
                    ctx.addEllipse(in: coin)
                    ctx.clip()
                    ctx.drawRadialGradient(g,
                                           startCenter: CGPoint(x: c.x - 3, y: c.y - 4), startRadius: 1,
                                           endCenter: c, endRadius: coin.width / 2,
                                           options: .drawsAfterEndLocation)
                    ctx.restoreGState()
                }
                ctx.setStrokeColor(UIColor(white: 0.35, alpha: 0.9).cgColor)
                ctx.setLineWidth(1)
                let coinR = coin.width / 2
                for k in 0..<28 {
                    let a = CGFloat(k) / 28 * 2 * .pi
                    ctx.move(to: CGPoint(x: c.x + cos(a) * (coinR - 2), y: c.y + sin(a) * (coinR - 2)))
                    ctx.addLine(to: CGPoint(x: c.x + cos(a) * coinR, y: c.y + sin(a) * coinR))
                }
                ctx.strokePath()
            }
