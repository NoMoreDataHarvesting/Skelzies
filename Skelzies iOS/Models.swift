...
// MARK: - Player control (human vs CPU) and difficulty tiers
...
enum CPUDifficulty: String, CaseIterable, Identifiable {
    ...
    /// Max aim error, in radians.
    var angleNoise: CGFloat {
        switch self {
        case .easy:   return 0.14
        case .medium: return 0.075
        case .hard:   return 0.035
        }
    }

    /// Max power error, as a fraction of the intended launch speed.
    var powerNoise: CGFloat {
        switch self {
    ...
    /// Chance of a completely flubbed shot (random power + wild angle).
    var flubChance: Double {
        switch self {
    ...
    /// Think time before the CPU fires.
...
enum PlayerControl: Equatable {
    ...
    var isCPU: Bool {
        if case .cpu = self { return true }
        return false
    }

    var difficulty: CPUDifficulty? {
        if case .cpu(let d) = self { return d }
        return nil
    }
}
